{
  ...
  description = "A minimalistic evil state";
  inputs = rec {
    bundle = {
  ...
  outputs = inputs@{ self, flake-utils, ... }:
    ...
    inputs.bundle.mkOutputs.emacs {
      inherit inputs self;
      pname = "evil-evilified-state";
      callPackage = { emacs }:
        iron.mkEmacsPackage {
          inherit self inputs emacs;
          package = rec {
            src = ./.;
            propagatedUserEnvPkgs = with emacs.pkgs; [ bind-map evil ];
            meta.description = "A minimalistic evil state";
          };
        };
    } { };
}
